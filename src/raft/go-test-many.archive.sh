...
logs=$(find /home/sulis/Documents/6.824/output -maxdepth 1 -name 'test-*.log' -type f -printf '.' | wc -c)
if [ "$logs" -eq 0 ]; then
    success=0
    failed=0
else
    success=$(grep -E '^PASS$' /home/sulis/Documents/6.824/output/test-*.log | wc -l)
    ((failed = logs - success))
fi
	...
	if ! wait "$1"; then
		if command -v notify-send >/dev/null 2>&1 &&((failed == 0)); then
			notify-send -i weather-storm "Tests started failing" \
				"$(pwd)\n$(grep FAIL: -- /home/sulis/Documents/6.824/output/*.log | sed -e 's/.*FAIL: / - /' -e 's/ (.*)//' | sort -u)"
		fi
		((failed += 1))
	else
		((success += 1))
...
cleanup() {
	for pid in "${waits[@]}"; do
		kill "$pid"
		wait "$pid"
		rm -rf "/home/sulis/Documents/6.824/output/test-${is[0]}.err" "/home/sulis/Documents/6.824/output/test-${is[0]}.log"
		is=("${is[@]:1}")
	done
	exit 0
}
...
for i in $(seq "$((success+failed+1))" "$runs"); do
	...
	if [[ -z "$test" ]]; then
		./output/tester -test.v 2> "/home/sulis/Documents/6.824/output/test-${i}.err" > "/home/sulis/Documents/6.824/output/test-${i}.log" &
		pid=$!
	else
		./output/tester -test.run "$test" -test.v 2> "/home/sulis/Documents/6.824/output/test-${i}.err" > "/home/sulis/Documents/6.824/output/test-${i}.log" &
		pid=$!
	fi
